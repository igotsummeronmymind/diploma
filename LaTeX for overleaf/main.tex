...
Roboflow License Plates). Детектор номерных знаков YOLOv8n достиг
mAP@0.50\,=\,0.889. Классификатор цвета ResNet18 достиг Accuracy
top-1\,=\,0.935. Классификатор типа кузова ResNet18~--- Accuracy
top-1\,=\,0.771. EasyOCR обеспечивает CRR\,=\,0.975 на синтетических
...
(по валидационной метрике) модель сохраняется:
\begin{equation}
  \mathrm{stop~if}\quad
...
\begin{itemize}
  ...
  \item \textbf{50 эпох:} при объёме датасета $\approx$6000 изображений
...
\begin{itemize}
  ...
  \item \textbf{20 эпох, разморозка на эпохе~5:}
    ...
    backbone~--- accuracy на валидации перешагнула с 0.78 до 0.93
...
\begin{itemize}
  ...
  \item \textbf{Early stopping, patience=5:}
    ...
    до 0.92, тогда как на val~--- снизилась. Лучшая модель
  ...
  \item \textbf{Разбивка CompCars 70/15/15\%:} стратифицированная
    выборка обеспечивает равномерное представление всех 6 классов
    в каждом разделе, что особенно важно при дисбалансе классов
...
\midrule
Accuracy top-1 & 0.624 & \textbf{0.771} \\
Accuracy top-3 & --- & 0.974 \\
Macro F1       & --- & 0.770 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{F1-score по классам типа кузова (val, ResNet18)}
...
\textbf{Класс} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} & \textbf{N} \\
\midrule
sedan     & 0.644 & 0.576 & 0.608 & 151 \\
suv       & 0.866 & 0.942 & 0.902 & 137 \\
hatchback & 0.726 & 0.755 & 0.740 & 151 \\
van       & 0.715 & 0.688 & 0.701 & 157 \\
truck     & 0.953 & 0.905 & 0.929 & 158 \\
minibus   & 0.715 & 0.774 & 0.743 & 146 \\
\midrule
\textbf{Macro avg} & 0.770 & 0.773 & 0.770 & 900 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Анализ ошибок по классам.}
Наиболее низкий F1 у класса \textit{sedan} (0.608): седаны
...
в идентичных условиях: одинаковое разбиение CompCars (70/15/15\%),
...
\toprule
\textbf{Модель} & \textbf{Params, M} & \textbf{FLOPS, G} & \textbf{Val Acc} & \textbf{FPS (T4)} & \textbf{.pth, MB} \\
\midrule
\textbf{ResNet18}    & 11.7 & 1.8 & \textbf{0.771} & \textbf{317.2} & $\approx 45$ \\
MobileNetV2 &  3.4 & 0.3 & 0.624 & 163.6 & $\approx 14$ \\
\bottomrule
\end{tabularx}
\end{table}
...
Классификация кузова& ResNet18 (CompCars)       & Accuracy top-1 & \textbf{0.771} \\
...
\begin{enumerate}
  ...
  \item \textbf{Детектор номерных знаков.}
        Fine-tuning YOLOv8n на датасете Roboflow LP (50 эпох, batch=16,
  ...
  \item \textbf{Классификация типа кузова.}
        ResNet18 достигает Accuracy top-1\,=\,\textbf{0.771}.
        Сравнение с MobileNetV2 (таблица~\ref{tab:arch_comparison})
        показывает, что ResNet18 превосходит MobileNetV2 на 14.7 п.п.
        по точности при более высоком FPS на GPU~T4
        (317.2 против 163.6), что обусловлено лучшей
        оптимизацией стандартных свёрток в CUDA по сравнению
        с depthwise-операциями MobileNetV2.
        Наиболее сложный класс~--- \textit{sedan} (F1\,=\,0.608)
