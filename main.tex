...
номерных знаков, классификация цвета автомобиля, тип кузова, YOLO26, ResNet18,
...
Предложена конвейерная архитектура: YOLO26n~--- детекция ТС и номерных знаков,
...
mAP@0.50\,=\,0.9669; классификатор цвета~--- Accuracy\,=\,0.936;
классификатор типа кузова~--- Accuracy\,=\,0.806;
...
параметров превосходит предыдущее поколение по точности на целевом домене
...
\begin{enumerate}
  ...
  \item Провести сравнительный эксперимент ResNet18 vs ResNet50 vs EfficientNet-B0.
\end{enumerate}
...
доменам (например, номерные знаки имеют специфическое соотношение сторон
$\approx$4:1, плохо покрываемое универсальными COCO-якорями). YOLO26
развивает эту линию: помимо отказа от якорей, модель убирает и
постобработку NMS, выполняя детекцию end-to-end
(подраздел~\ref{subsec:anchor_free}).

\subsection{Backbone: CSP-сеть}
\label{subsec:backbone}

Backbone YOLO26 основан на принципе \textbf{Cross-Stage Partial
...
CSP-блоки с несколькими bottleneck-ветвями (развитие подхода C3$\to$C2f
из поколений 2020--2023~годов) сохраняют все промежуточные feature maps
в финальной конкатенации, что увеличивает число независимых путей
градиента при незначительном росте параметров. В YOLO26 backbone
дополнительно оптимизирован под сокращение FLOPs: версия nano использует
лишь 5.4~GFLOPs (против 8.7 у YOLOv8n) при более высокой точности на COCO.

\subsection{Neck: Path Aggregation Network}
\label{subsec:neck}

Для обнаружения объектов разных масштабов (мелкие~--- номерные
знаки вдали, крупные~--- автомобили вблизи) YOLO26 использует
...
YOLO26 напрямую предсказывает расстояния от центра ячейки до
...
\textbf{Отказ от Distribution Focal Loss.} В поколении 2023~года
координаты регрессировались через DFL~--- предсказание дискретного
распределения вероятностей по бинам с последующим взятием
математического ожидания. YOLO26 отказывается от DFL и регрессирует
координаты напрямую. Это убирает дополнительную ветвь в голове и
упрощает граф вычислений: ускоряется инференс (по данным Ultralytics
до 43\% на CPU) и устраняются проблемы при экспорте в ONNX/TensorRT,
где слой DFL ранее требовал отдельной поддержки.

\textbf{End-to-end инференс без NMS.} Классические детекторы (в том числе
YOLOv8) выдают для каждого объекта множество перекрывающихся боксов,
которые затем прореживаются алгоритмом Non-Maximum Suppression (NMS)
по порогу IoU. NMS~--- непараллелизуемая постобработка с собственной
задержкой и гиперпараметром (порогом IoU). YOLO26 обучается с
назначением меток <<один-к-одному>>, благодаря чему голова сразу выдаёт
финальный непересекающийся набор детекций \emph{без} NMS. Это снижает
сквозную задержку и делает время инференса детерминированным~--- не
зависящим от числа кандидатов в кадре.

\subsection{Функция потерь и оптимизация}
\label{subsec:yolo_loss}

После отказа от DFL функция потерь YOLO26 объединяет два компонента~---
...
учитывает три геометрических аспекта совпадения боксов:
\begin{equation}
...
В отличие от простого IoU-loss, CIoU даёт ненулевой градиент при
непересекающихся боксах и сходится быстрее.
...
Дополнительно YOLO26 вводит два механизма обучения:
\begin{itemize}[nosep]
  \item \textbf{ProgLoss} (Progressive Loss Balancing)~--- прогрессивно
    балансирует вклад $\mathcal{L}_{\mathrm{box}}$ и
    $\mathcal{L}_{\mathrm{cls}}$ по ходу обучения, стабилизируя
    сходимость на ранних эпохах;
  \item \textbf{STAL} (Small-Target-Aware Label assignment)~---
    назначение положительных меток с учётом мелких объектов,
    повышающее их полноту (recall); это релевантно для удалённых
    номерных знаков.
\end{itemize}
...
Ultralytics, которая для датасетов объёмом менее 10\,000 изображений
автоматически подбирает алгоритм оптимизации и начальную скорость
обучения. При этом для YOLO26 в качестве базового алгоритма введён
\textbf{MuSGD}~--- гибрид Muon и SGD, дающий более стабильную и быструю
сходимость, чем чистый SGD с импульсом. Автоматический подбор при
ограниченном объёме целевых данных и коротком расписании обучения
обеспечивает устойчивую сходимость без ручной настройки скорости
обучения.
...
В данной работе early stopping с patience=7 применяется
для классификатора кузова, что предотвратило переобучение~---
лучшая модель получена на эпохе~27, обучение остановлено на эпохе~34.
...
Набор аугментаций различается для двух задач. Для классификатора
\emph{цвета} применяется базовый набор (таблица~\ref{tab:augmentation})
с сохранением цветовых искажений (\texttt{ColorJitter}), ключевых для
устойчивости к освещению. Для классификатора \emph{типа кузова} тот же
набор усилен под доменный разрыв CompCars\,$\to$\,улица: добавлены
\texttt{RandomPerspective} (distortion~0.3, $p=0.5$)~--- имитация уличных
ракурсов 3/4; \texttt{GaussianBlur} ($p=0.25$)~--- низкое разрешение и смаз
удалённых кадров; \texttt{RandomErasing} ($p=0.3$)~--- частичные
перекрытия. \texttt{ColorJitter} при этом сохранён (насыщенность снижена
до 0.15), нижняя граница \texttt{RandomResizedCrop}~--- 0.72, а перед
FC-слоем добавлены \texttt{Dropout}~0.5 и сглаживание меток
(label smoothing~0.1) как дополнительная регуляризация.
...
в режиме feature extraction (0.666). Платой выступает практически
...
\begin{itemize}
  ...
  \item \textbf{Оптимизатор \texttt{optimizer=auto}:}
    Ultralytics для датасета такого объёма автоматически подбирает
    алгоритм оптимизации и начальную скорость обучения. Для YOLO26
    штатным оптимизатором является MuSGD (гибрид Muon и SGD). Автоподбор
    при небольшом целевом датасете и коротком обучении обеспечивает
    устойчивую сходимость без ручной настройки LR.
...
\label{tab:plate_det}
% TODO(re-run): YOLO26n mAP@0.50=0.9669 и строки Median IoU / IoU>=0.50 / IoU>=0.75
% взяты из прошлого прогона; в текущем nb01 executed только mAP@0.50=0.9554 (ячейка 27),
% а ячейки сравнения 45/47/49 не запускались. Пересчитать на YOLO26n при доступе к Colab
% и синхронизировать с tab:yolo_compare, tab:final_summary, аннотацией и заключением.
\rowcolors{2}{rowgray}{white}
\begin{tabular}{lccc}
\toprule
...
Выбранный детектор номеров \textbf{YOLO26n} достиг mAP@0.50\,=\,0.9669~---
выше дообученного YOLOv8n (0.9554) и кратно выше базовой предобученной на
COCO модели ($\approx$0.503). Это соответствует уровню промышленных
ANPR-систем. Детальный анализ распределения IoU, выполненный для
дообученного детектора, даёт медианный IoU\,=\,0.809 при доле
IoU\,$\geq$\,0.75 в 67.6\%: лишь треть боксов имеет умеренное расхождение,
обусловленное тенями и бликами на краях номерного знака; 94.0\% боксов
преодолевают порог IoU\,=\,0.50, что достаточно для корректной обрезки
кропа под OCR.

\subsection{Сравнение с YOLOv8n}
\label{subsec:yolo_compare}

Чтобы обосновать выбор YOLO26n, на том же датасете (542/126 train/test)
в тех же условиях дообучена модель предыдущего поколения YOLOv8n
(3.2M параметров, 8.7~GFLOPs)~--- как базовая линия (baseline).

\begin{table}[H]
\centering
\caption{Сравнение дообученных архитектур на тесте (126~изображений)}
\label{tab:yolo_compare}
% TODO(re-run): см. примечание у tab:plate_det — 0.9669 пока без executed-подтверждения.
\rowcolors{2}{rowgray}{white}
\begin{tabular}{lcc}
\toprule
...
YOLO26n превысил YOLOv8n на 1.15~п.п. mAP@0.50 при меньшем числе
параметров (2.4M против 3.2M) и FLOPs (5.4 против 8.7~GFLOPs). Хотя при
сравнении \emph{предобученных} моделей на одном тестовом кадре YOLO26n
оказался несколько медленнее (16.0~мс против 12.6~мс/кадр на GPU), после
fine-tuning он достигает более высокой точности в целевом домене~--- то
есть новая архитектура эффективнее использует меньшую ёмкость. Поэтому
в качестве финального детектора (и ТС, и номеров) выбран YOLO26n; выбор
лучшей модели для конвейера выполняется автоматически по mAP@0.50
(ноутбук~01).
...
и \textit{orange} (F1 = 0.896). Silver при пониженном Recall (0.844)
...
\begin{itemize}
  ...
  \item \textbf{Early stopping, patience=7:}
    ...
    (sedan vs.\ hatchback, van vs.\ minibus). Обучение
    остановлено на эпохе~34; лучшая модель зафиксирована
    на эпохе~27 (Val~Acc\,=\,0.804), после чего точность вышла
    на плато без признаков переобучения.
...
Общая точность top-1 остаётся практически идентичной baseline (0.789 vs 0.787),
однако F1 класса \textit{sedan}~--- наиболее проблемного~--- вырос
с 0.610 до 0.645 ($+3.5$~п.п.). Recall sedan улучшился с 0.523 до 0.656:
...
\textbf{Доработка финальной модели.}
Поверх конфигурации Mixup $\alpha=0.2$ финальная модель получила ряд
уточнений: (1)~очищена разметка класса \textit{sedan}~--- исключены
исходные типы \textit{sports} и \textit{convertible}; (2)~аугментации
усилены под доменный разрыв CompCars\,$\to$\,улица~--- добавлены
\texttt{RandomPerspective}, \texttt{GaussianBlur} и \texttt{RandomErasing}
при сохранённом \texttt{ColorJitter} (нижняя граница
\texttt{RandomResizedCrop}~--- 0.72);
(3)~лимит эпох повышен до 40 (early stopping сработал на 34-й);
(4)~на этапе оценки добавлен TTA~--- усреднение softmax по исходному
изображению и его горизонтальному отражению. В совокупности эти
уточнения повысили Accuracy top-1 с 0.789 до \textbf{0.806}.

Примечательно, что очистка класса \textit{sedan} \emph{не} улучшила
сам \textit{sedan}: его F1 снизился с 0.645 до 0.593, а Recall~--- до 0.562.
Удалив визуально яркие спорткары и кабриолеты, мы оставили в классе
только «трудные» седаны, почти неотличимые сзади от хетчбэков. Зато
соседний \textit{hatchback}, переставший «засоряться» чужими силуэтами,
вырос по F1 с 0.732 до 0.803. Итоговый прирост точности обеспечен
именно классами hatchback, truck и minibus. Это подтверждает, что
граница sedan\,$\leftrightarrow$\,hatchback~--- фундаментальное
ограничение визуальных данных, а не разметки.
...
\midrule
\textbf{Macro avg} & 0.793 & 0.794 & 0.792 & 900 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Анализ ошибок по классам.}
Наиболее низкий F1 у класса \textit{sedan} (0.593): седаны
...
Класс \textit{sedan} имеет Recall = 0.562:
из 128 тестовых седанов 56 отнесены к другим классам (преимущественно к hatchback).
Симметрично, \textit{hatchback} имеет Recall = 0.768:
из 151 хетчбэка 35 классифицированы неверно (преимущественно как sedan и van).
Суммарно пара sedan/hatchback генерирует $\approx$91 ошибку из
$\sim$175 общих ложноотрицательных~--- около~52\,\% всех ошибок модели.
...
(\textit{van}~--- Recall 0.701, \textit{minibus}~--- Recall 0.796).

Классы \textit{truck} (Recall = 1.000, 0 FN) и \textit{suv} (Recall = 0.940, 9 FN)
...
архитектур стало бы confounding factor: модель с более ёмким
...
сравнении ResNet18 показывает только результат FC-головы (0.616);
итоговая задеплоенная модель использует полное двухфазное
дообучение и достигает 0.806 (таблица~\ref{tab:body_metrics}).
...
\midrule
ResNet18              & 11.2 & 1.80 & 0.616          & \textbf{330.8} & 44.7 \\
\textbf{ResNet50}     & 23.5 & 4.10 & \textbf{0.666} & 180.2          & 94.1 \\
EfficientNet-B0       &  4.0 & 0.39 & 0.637          & 134.1          & 16.1 \\
\bottomrule
\end{tabularx}
\end{table}

В режиме feature extraction наибольшую точность показывает
\textbf{ResNet50} (0.666)~--- более ёмкий backbone извлекает
более информативные признаки. Однако разрыв с ResNet18 (0.616)
и EfficientNet-B0 (0.637) невелик (около 3--5~п.п.), тогда как
по скорости \textbf{ResNet18 опережает ResNet50 почти вдвое}
(330.8 против 180.2~FPS) при вдвое меньшем числе параметров.
Парадоксально, что \textbf{EfficientNet-B0 при наименьших FLOPS
(0.39G) оказалась самой медленной} (134.1~FPS): depthwise-свёртки
плохо утилизируют параллельные блоки GPU, поэтому теоретическая
экономия операций не переходит в реальную скорость.

Поскольку при полном дообучении ResNet18 достигает 0.806 и при
...
($\approx$5~п.п.) не оправдывает удвоения задержки и размера модели.
...
Для качественной оценки end-to-end конвейер запущен на пяти реальных
фотографиях транспортных средств, различающихся маркой, цветом, типом
кузова, ракурсом и условиями съёмки (рисунок~\ref{fig:single_infer}).
На каждом изображении выполняются детекция ТС, классификация цвета и
типа кузова и распознавание номерного знака; сводка приведена в
таблице~\ref{tab:single_infer}.

\begin{figure}[H]
\centering
\begin{subfigure}{0.49\textwidth}\centering
  \includegraphics[height=4.8cm]{inf0.jpg}\caption{Aurus Senat}
\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}\centering
...
\medskip
\begin{subfigure}{0.49\textwidth}\centering
  \includegraphics[height=5.8cm]{inf1.jpeg}\caption{Porsche Macan}
\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}\centering
  \includegraphics[height=5.8cm]{inf2.jpeg}\caption{Mercedes-Benz S-класс}
\end{subfigure}

\medskip
\begin{subfigure}{0.7\textwidth}\centering
  \includegraphics[height=4.8cm]{inf4.jpeg}\caption{Toyota Yaris}
\end{subfigure}
...
($\checkmark$~--- верно, $\times$~--- ошибка; в скобках~--- истинный класс)}
...
\midrule
Aurus Senat     & black\,$\checkmark$  & hatchback (sedan)\,$\times$ & Е955КХ97\,$\checkmark$  \\
BMW X5          & white\,$\checkmark$  & van (suv)\,$\times$         & ЕОО8КХ62\,$\times$\textsuperscript{$\dagger$} \\
Porsche Macan   & black\,$\checkmark$  & van (suv)\,$\times$         & Е258КР790\,$\checkmark$ \\
Mercedes-Benz S & black\,$\checkmark$  & hatchback (sedan)\,$\times$ & С697НЕ977\,$\checkmark$ \\
Toyota Yaris    & silver\,$\checkmark$ & sedan (hatchback)\,$\times$ & О947НХ150\,$\checkmark$ \\
\midrule
\textbf{Итого}  & \textbf{5/5} & \textbf{0/5} & \textbf{4/5} \\
\bottomrule
\end{tabular}
\end{table}
...
корректно, но точного совпадения нет; в видеоконвейере такая ошибка
устраняется коррекцией по маске формата (подраздел~\ref{subsec:ocr_errors}).

\textbf{Цвет (5/5).} Классификатор корректен на всех изображениях,
включая три тёмных бликующих кузова (Aurus, Mercedes, Macan), белый
(BMW~X5) и серебристый (Yaris). Это подтверждает устойчивость
классификатора цвета (top-1\,=\,0.936, таблица~\ref{tab:final_summary})
к реальному освещению и бликам.

\textbf{Номер (4/5).} Четыре номера распознаны точно: Е955КХ97 (Aurus),
Е258КР790 (Macan), С697НЕ977 (Mercedes), О947НХ150 (Yaris). На всех
снимках это крупные фронтальные пластины ($\approx$200--300~px по
ширине)~--- благоприятный для OCR режим (CRR\,=\,0.975,
подраздел~\ref{subsec:ocr_errors}). Единственная ошибка~--- на BMW~X5,
где две нулевые цифры прочитаны как буква «О» (O\,↔\,0). Показательно, что
одиночный инференс использует базовый OCR (бинаризация Оцу) без коррекции
по маске формата: именно эта коррекция в видеоконвейере
(подраздел~\ref{subsec:ocr_errors}) на цифровых позициях заменила бы «О»
на «0» и восстановила номер. Латинские гомоглифы в подписях
(Е/К/Х/С/Н/О/Р отображаются как E/K/X/C/H/O/P) косметические и значение
номера не искажают.

\textbf{Тип кузова (0/5).} Классификатор ошибся на всех изображениях,
однако показательно, что все ошибки~--- между \emph{соседними} классами:
длинные седаны (Aurus, Mercedes~S) отнесены к \textit{hatchback},
внедорожники (BMW~X5, Porsche~Macan)~--- к \textit{van}, а малолитражный
хетчбэк (Toyota~Yaris)~--- к \textit{sedan}. Грубый силуэт улавливается,
но смежные формы путаются. Причины согласуются с ранее выявленными:
(а)~доменный разрыв~--- CompCars содержит преимущественно каталожные
ракурсы, тогда как тестовые фото сняты с переднего 3/4;
(б)~фронтальный ракурс скрадывает заднюю часть кузова, по которой и
различаются sedan/hatchback/suv;
(в)~задокументированный потолок классификатора (top-1\,=\,0.806,
таблица~\ref{tab:final_summary}) и известные путаницы
\emph{sedan\,↔\,hatchback}, \emph{suv\,↔\,van}.

Таким образом, тест на одиночных изображениях подтверждает выводы по
отдельным модулям: классификация цвета и распознавание номера надёжно
работают на реальных данных, а классификатор типа кузова остаётся
слабейшим звеном и требует доменной адаптации (дообучение на уличных
ракурсах)~--- это направление дальнейшей работы.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{номер}~--- наиболее частый среди \emph{формат-валидных}
    ...
    К каждому кропу применяются контрастная нормализация (CLAHE) и
    коррекция по маске формата (подраздел~\ref{subsec:ocr_errors}).
\end{itemize}
...
% TODO(re-run): см. примечание у tab:plate_det — 0.9669 (детекция номеров) пока без executed-подтверждения.
\rowcolors{2}{rowgray}{white}
...
Детекция номеров    & YOLOv8n (fine-tuned, baseline) & mAP@0.50    & 0.9554 \\
Классификация цвета & ResNet18 (VCoR)            & Accuracy top-1 & \textbf{0.936} \\
Классификация цвета & ResNet18 (VCoR)            & Accuracy top-3 & 0.993 \\
Классификация кузова& ResNet18 (CompCars)        & Accuracy top-1 & \textbf{0.806} \\
...
\textbf{Меры по снижению ошибок OCR в видеоконвейере.}
Выявленные ошибки частично компенсируются на этапе видеоинференса
(функция \texttt{read\_plate\_strict}) двумя мерами~--- предобработкой
кропа и коррекцией предсказания по маске формата.

\emph{Предобработка кропа.} Перед распознаванием кроп номера переводится
в оттенки серого и подвергается локальному выравниванию контраста
методом CLAHE (Contrast Limited Adaptive Histogram Equalization,
\texttt{clipLimit}\,=\,2.0, сетка $4\times4$ тайлов), что частично
компенсирует блики и неравномерное освещение~--- фактор деградации,
отмеченный выше. Мелкие кропы дополнительно увеличиваются бикубической
интерполяцией с целочисленным коэффициентом до ориентировочной ширины
$\sim$300~px, поднимая эффективное разрешение для CRNN.

\emph{Коррекция по маске формата.} Поскольку все наиболее частые ошибки
относятся к парам \emph{кириллица\,↔\,цифра}
(таблица~\ref{tab:ocr_confusions}), а формат номера РФ однозначно задаёт
тип символа на каждой позиции (маска \texttt{Б\,Ц\,Ц\,Ц\,Б\,Б\,Ц\,Ц\,Ц},
где Б~--- буква, Ц~--- цифра), распознанная строка проецируется на эту
маску: на цифровых позициях буквы заменяются визуально близкими цифрами,
на буквенных~--- наоборот (таблица~\ref{tab:ocr_corrections}). После
подстановки строка повторно проверяется регулярным выражением формата
и при успешной проверке засчитывается как валидный номер.

\begin{table}[H]
\centering
\caption{Подстановки при коррекции по маске формата (обратны частым
ошибкам из таблицы~\ref{tab:ocr_confusions})}
\label{tab:ocr_corrections}
\rowcolors{2}{rowgray}{white}
\begin{tabular}{cc}
\toprule
\textbf{Цифровая позиция: буква\,$\to$\,цифра} &
\textbf{Буквенная позиция: цифра\,$\to$\,буква} \\
\midrule
О\,$\to$\,0,\quad Б\,$\to$\,6,\quad В\,$\to$\,8 & 0\,$\to$\,О,\quad 8\,$\to$\,В,\quad 6\,$\to$\,Б \\
З\,$\to$\,3,\quad Т\,$\to$\,7                    & 3\,$\to$\,З,\quad 7\,$\to$\,Т \\
А\,$\to$\,4,\quad У\,$\to$\,4                    & 4\,$\to$\,А \\
\bottomrule
\end{tabular}
\end{table}

Обе меры носят эвристический характер: они не исправляют ошибки
локализации символов детектором CRAFT, но восстанавливают номера,
не прошедшие проверку формата исключительно из-за путаницы
\emph{кириллица\,↔\,цифра}. Так как подстановки применяются только
к позициям фиксированного типа, риск превратить корректный символ
в ошибочный минимален.
...
На пяти тестовых фотографиях (подраздел~\ref{subsec:single_image})
конвейер верно определил цвет у всех пяти ТС (5/5) и точно распознал
номер у четырёх из пяти (4/5; единственная ошибка~--- путаница
O\,↔\,0), тогда как тип кузова не был распознан ни разу (0/5), причём
все ошибки~--- между соседними классами (sedan\,↔\,hatchback,
suv\,↔\,van). Это подтверждает работоспособность модулей цвета и OCR
в режиме end-to-end и указывает на классификатор типа кузова как на
приоритетное направление доработки.
...
эксперимент ResNet18 vs ResNet50 vs EfficientNet-B0. Основные результаты приведены ниже.
...
\begin{enumerate}
  ...
  \item \textbf{Детектор номерных знаков.}
        Fine-tuning YOLO26n на датасете Russian license plates (Kaggle,
        668 изображений, 50 эпох, batch=16, \texttt{optimizer=auto})
        позволил достичь mAP@0.50\,=\,\textbf{0.9669}. На тех же данных
        модель предыдущего поколения YOLOv8n (baseline) дала 0.9554, то
        есть YOLO26n превзошёл её на 1.15~п.п. при меньшем числе параметров
        (2.4M против 3.2M) и был выбран финальной моделью-детектором.
        Медианный IoU\,=\,0.809 и доля боксов с IoU\,$\geq$\,0.50 в 94.0\,\%
        подтверждают высокую точность локализации. Это подтверждает высокую
        эффективность transfer learning при ограниченном объёме целевых
        данных.

  \item \textbf{Классификация цвета.}
        ResNet18, дообученный на VCoR за 20 эпох с двухфазной стратегией,
        достигает Accuracy top-1\,=\,\textbf{0.936} и top-3\,=\,0.993.
        Наиболее сложными классами оказались \textit{silver} (F1\,=\,0.867)
        и \textit{orange} (F1\,=\,0.896) из-за визуального сходства
  ...
  \item \textbf{Классификация типа кузова.}
        ...
        sedan, аугментации под форму и TTA) достигает
        Accuracy top-1\,=\,\textbf{0.806} и top-3\,=\,0.969.
        Сравнение архитектур в едином режиме feature extraction
        (таблица~\ref{tab:arch_comparison}) показало, что наибольшую
        точность даёт ResNet50 (0.666), однако ResNet18 опережает
        её по скорости почти вдвое (330.8 против 180.2~FPS на GPU~T4)
        при отставании в точности всего на 5~п.п.; поэтому для
        реального видеоконвейера выбрана ResNet18. Характерно, что
        EfficientNet-B0 при наименьших FLOPS (0.39G) оказалась самой
        медленной (134.1~FPS): depthwise-свёртки слабо утилизируют GPU.
        Наиболее сложный класс~--- \textit{sedan} (F1\,=\,0.593)
