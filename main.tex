...
\emph{цвета} сохраняются цветовые искажения (ColorJitter), но
исключаются преобразования, разрушающие цвет. Напротив, для
классификатора \emph{типа кузова}, где признаком служит геометрия
силуэта, а не цвет, нижняя граница \texttt{RandomResizedCrop} повышена
до 0.78 (чтобы случайный кроп не срезал крышу и заднюю часть~--- ключевые
для различения sedan/hatchback), а аугментации, искажающие цвет
(\texttt{RandomGrayscale}, изменение насыщенности), исключены как
бесполезные для формы.
...
переориентированы на форму, а не цвет~--- нижняя граница
\texttt{RandomResizedCrop} повышена до 0.78 (чтобы не срезать силуэт),
удалены \texttt{RandomGrayscale} и случайное изменение насыщенности;
(3)~лимит эпох повышен с 30 до 40 (early stopping сработал на 34-й);
