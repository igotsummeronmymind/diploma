...
mAP@0.50\,=\,0.9554; классификатор цвета~--- Accuracy\,=\,0.936;
классификатор типа кузова~--- Accuracy\,=\,0.830;
...
\begin{table}[H]
\centering
\caption{Метрики детектора номерных знаков YOLO26n (тест, 126~изображений)}
\label{tab:plate_det}
\rowcolors{2}{rowgray}{white}
\begin{tabular}{lc}
\toprule
\textbf{Конфигурация} & \textbf{mAP@0.50} \\
\midrule
Предобученная на COCO (zero-shot)   & $\approx$0.503 \\
\textbf{Дообученная YOLO26n}         & \textbf{0.9554} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Анализ результатов.}
Дообученный детектор номеров \textbf{YOLO26n} достиг mAP@0.50\,=\,0.9554~---
кратно выше базовой предобученной на COCO модели ($\approx$0.503), не
видевшей российских номеров. Это соответствует уровню промышленных
ANPR-систем и подтверждает высокую эффективность transfer learning при
ограниченном объёме целевых данных (542~обучающих изображения). Достигнутой
точности локализации достаточно для корректной обрезки кропа номера под
последующее OCR-распознавание.

% TODO(re-run): nb01 пересобран и пока без выводов — графиков детектора нет.
% После прогона в Colab сохранить из nb01 и вставить сюда:
%   figs/det_pr_curve.png      — PR-кривая дообученного YOLO26n,
%   figs/det_train_curves.png  — кривые обучения (box/cls/dfl loss, mAP),
%   figs/det_samples.png       — примеры детекций номеров на тесте.
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.8\textwidth]{det_pr_curve.png}
%   \caption{PR-кривая дообученного детектора номеров YOLO26n (тест)}
%   \label{fig:det_pr}
% \end{figure}
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=\textwidth]{det_samples.png}
%   \caption{Примеры детекции номерных знаков дообученным YOLO26n}
%   \label{fig:det_samples}
% \end{figure}
...
Дополнительное преимущество YOLO26n~--- сквозной (end-to-end) инференс без
постобработки NMS, что упрощает конвейер и стабилизирует задержку. Хотя при
...
дообучения он достигает в целевом домене mAP@0.50\,=\,0.9554
(таблица~\ref{tab:plate_det}) при меньшей ёмкости. Поэтому в качестве
финального детектора (и ТС, и номеров) выбран YOLO26n; выбор лучшей модели
для конвейера выполняется автоматически по mAP@0.50 (ноутбук~01).
...
сторону доминирующего класса. Характерные изображения каждого класса
приведены на рисунке~\ref{fig:color_samples}: видно как разнообразие
ракурсов и условий съёмки, так и принципиальную трудность ахроматических
классов (\textit{black}, \textit{white}, \textit{silver}), границы между
которыми зависят от освещения.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{color_eda_samples.png}
\caption{Примеры изображений VCoR по девяти целевым классам цвета}
...
автомобильного домена. Кривые потерь и точности
...
\textbf{Состав обучающей выборки.}
После маппинга исходных типов CompCars в шесть целевых классов и
ограничения \texttt{MAX\_PER\_CLASS} выборка близка к сбалансированной
(рисунок~\ref{fig:body_dist}). Характерные изображения каждого типа
кузова приведены на рисунке~\ref{fig:body_samples}; уже визуально видна
ключевая трудность задачи~--- близость силуэтов легковых классов
(\textit{hatchback}, \textit{sedan}, \textit{suv}) в отличие от
принципиально иных \textit{truck} и \textit{minibus}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{body_eda_samples.png}
...
(3)~лимит эпох повышен до 40 (обучение прошло все 40 эпох,
early stopping не сработал, лучшая модель~--- эпоха~34);
...
уточнения повысили Accuracy top-1 с 0.789 до \textbf{0.830}.

Наибольший прирост дало именно усиление аугментаций под доменный
разрыв (перспективные искажения, размытие, более агрессивный
\texttt{ColorJitter} и стирание): геометрические и световые искажения
(рисунок~\ref{fig:body_aug}) вынудили модель опираться на устойчивые
силуэтные признаки кузова, а не на ракурс и освещение каталожных фото
CompCars. Основной выигрыш пришёлся на трудноразличимый кластер легковых
классов \textit{hatchback}\,/\,\textit{sedan}\,/\,\textit{suv}
...
\caption{Аугментации обучающей выборки \texttt{TRAIN\_TF} под доменный
разрыв CompCars\,$\to$\,улица: перспективные искажения, цветовые сдвиги,
...
недообучения (см.\ обсуждение в подразделе про Mixup).
...
остальных (высокий клиренс и рамочная конструкция у truck, вытянутый
коробчатый кузов у minibus). Accuracy top-3 = 0.969 означает, что
...
\begin{itemize}[nosep]
  \item \textit{sedan} (Recall = 0.748): из 151 седана $\approx$38 ошибок,
    из них $\approx$21 отнесены к \textit{hatchback}~--- это крупнейшее
    внедиагональное ребро всей матрицы (0.14);
  \item \textit{hatchback} (Recall = 0.719): из 128 хетчбэков $\approx$36
    ошибок, причём в основном в сторону \textit{suv} ($\approx$15, 0.12)
    и \textit{van} ($\approx$10), и лишь $\approx$6~--- в сторону sedan;
  \item \textit{suv} (Recall = 0.753): из 154 внедорожников $\approx$38
    ошибок, преимущественно как \textit{hatchback} ($\approx$15) и
    \textit{van} ($\approx$9).
\end{itemize}
Суммарно взаимные путаницы внутри тройки
\{\textit{hatchback}, \textit{sedan}, \textit{suv}\} дают $\approx$73 из
...
вариативный \textit{van} слабее (Recall = 0.796) и в $\approx$11~случаях
...
% TODO(re-run): таблица обновлена под исправленный порядок BODY_LABELS
% (тип кузова теперь 5/5). Изображения inf0..inf4 нужно перегенерировать
% в nb04 после фикса — на старых картинках прожжены прежние (ошибочные)
% подписи кузова. До перегенерации figure не отражает таблицу.
\begin{figure}[H]
\centering
...
\midrule
Aurus Senat     & black\,$\checkmark$  & sedan\,$\checkmark$     & Е955КХ97\,$\checkmark$  \\
BMW X5          & white\,$\checkmark$  & suv\,$\checkmark$       & ЕОО8КХ62\,$\times$\textsuperscript{$\dagger$} \\
Porsche Macan   & black\,$\checkmark$  & suv\,$\checkmark$       & Е258КР790\,$\checkmark$ \\
Mercedes-Benz S & black\,$\checkmark$  & sedan\,$\checkmark$     & С697НЕ977\,$\checkmark$ \\
Toyota Yaris    & silver\,$\checkmark$ & hatchback\,$\checkmark$ & О947НХ150\,$\checkmark$ \\
\midrule
\textbf{Итого}  & \textbf{5/5} & \textbf{5/5} & \textbf{4/5} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Тип кузова (5/5).} Классификатор корректно определил тип кузова
на всех пяти изображениях: седаны (Aurus, Mercedes~S), внедорожники
(BMW~X5, Porsche~Macan) и малолитражный хетчбэк (Toyota~Yaris). Это
согласуется с задокументированной точностью top-1\,=\,0.830
(таблица~\ref{tab:final_summary}): на крупных, хорошо освещённых
фронтальных снимках, ракурсом близких к обучающим, классификатор
уверенно различает даже смежные формы. Показательно, что здесь верно
разрешена и трудная пара \emph{sedan\,↔\,hatchback} (Aurus и Mercedes~S~---
седаны, Yaris~--- хетчбэк), хотя на валидации
(таблица~\ref{tab:body_per_class}) именно она остаётся основным
источником ошибок.

Таким образом, тест на одиночных изображениях показывает согласованную
работу всех трёх модулей на реальных данных: цвет и тип кузова определены
верно на всех пяти снимках, номер~--- на четырёх из пяти (единственная
ошибка O\,↔\,0 устраняется коррекцией по маске формата в видеоконвейере).
При этом снимки сделаны в благоприятных условиях (крупный фронтальный
ракурс, хорошее освещение); повышение устойчивости классификатора кузова
к произвольным уличным ракурсам и трудной паре sedan\,↔\,hatchback~---
направление дальнейшей доменной адаптации.
...
Детекция номеров    & \textbf{YOLO26n (fine-tuned)} & \textbf{mAP@0.50} & \textbf{0.9554} \\
...
конвейер верно определил цвет и тип кузова у всех пяти ТС (5/5 и 5/5)
и точно распознал номер у четырёх из пяти (4/5; единственная ошибка~---
путаница O\,↔\,0, устранимая коррекцией по маске формата в
видеоконвейере). Снимки сделаны в благоприятных условиях (крупный
...
направлениям дальнейшей доменной адаптации.
...
\begin{enumerate}
  ...
  \item \textbf{Детектор номерных знаков.}
        ...
        позволил достичь mAP@0.50\,=\,\textbf{0.9554} против $\approx$0.503
        у предобученной на COCO модели. YOLO26n выбран финальным детектором
        (и ТС, и номеров) как более эффективная архитектура: при меньшем
        числе параметров (2.4M против 3.2M у YOLOv8n), меньшей вычислительной
        сложности и сквозном инференсе без NMS он показывает более высокую
        точность на эталонном наборе COCO. Это подтверждает высокую
  ...
  \item \textbf{Классификация типа кузова.}
        ...
        sedan, аугментации под уличный домен и TTA) достигает
